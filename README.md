...
오늘 등록분 중 심각·높음·중간 등급이면서 NVD 공식 CWE(취약점 유형) 분류가 있는 CVE는 전부 대표로 뽑아 CVSS 점수·유형 태그·공격 조건(네트워크/로컬 접근, 난이도, 권한, 사용자 개입 필요 여부)과 함께 보여줍니다(낮음 등급은 대상이 아니며, 너무 많은 날은 등급·CVSS 순으로 상한까지만). 처음엔 1건만 보이고, 제목을 누르면 한국어 번역·원문과 함께 "AI 해설"(쉬운 해석, CWE를 근거로 한 발생 원인, 방지·완화 방법)이 펼쳐집니다 — 근거가 부족하면 지어내지 않고 해당 항목을 비워둡니다. "더보기"를 누르면 나머지가 5건씩 페이지네이션되고, 그 아래에는 CWE 분류가 없어 대표로 못 올라간 CVE가 번역·해설 없이 목록으로만 따로 보입니다.
...
[GitHub Actions: 매일 00:00 UTC(=09:00 KST) 실행, workflow_dispatch로 수동 실행도 가능]
        ...
        │  4) 심각·높음·중간 등급 대표 CVE마다 Gemini로 번역+해석+발생 원인(CWE 근거)+방지법을 한 번에 생성(실패 시 원문만 노출)
        ▼
