...
- **오늘 등록분 유형 분류** — 심각도가 평가된 CVE 전체의 설명 문구를 키워드 규칙(원격 코드 실행·인증 우회·SQL 인젝션 등)으로 분류해 막대 그래프로 보여줍니다. 번역 API는 안 쓰고, 규칙에 안 걸려 "기타"로 빠진 설명만 Gemini API 무료 티어로 한 번 더 재분류합니다(같은 유형 목록 안에서만 고르도록 강제).
