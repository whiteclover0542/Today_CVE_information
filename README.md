...
오늘 등록분 중 심각도 상위 CVE를 CVSS 점수·유형 태그·공격 조건(네트워크/로컬 접근, 난이도, 권한, 사용자 개입 필요 여부)·한국어 설명과 함께 보여줍니다. 처음엔 1건만 보이고 "더보기"를 누르면 나머지가 5건씩 페이지네이션됩니다. 각 카드에는 "AI 해설" 보기가 따로 있어 이 취약점을 쉽게 풀어 쓴 해석, 왜 발생했는지(NVD 공식 CWE 분류를 근거로 삼음), 어떻게 방지·완화하는지를 함께 볼 수 있습니다 — 근거가 부족하면 지어내지 않고 해당 항목을 비워둡니다.
