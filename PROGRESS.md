...
| 14 | 화면 디자인 정리 (시각화·데이터 구분·가독성) | ✅ | dataviz 스킬 기준으로 색상 체계 도입(CWE=파랑·제품=주황·추이=청록 고정 순서, 심각도=상태 팔레트), 방법론 설명 문단을 전부 접이식 `<details>`로 전환, 헤더 안내를 칩 버튼으로 정리. 검정 테마는 유지 |
...
1. 새 선정 로직(심각·높음·중간 전부 + 상한 50)과 디자인 개편을 커밋·push하고 `workflow_dispatch`로 재실행해 실제로 몇 건이 뽑히는지, Gemini 호출·배치 실행 시간이 문제없는지 확인
